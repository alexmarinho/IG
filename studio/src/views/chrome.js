  ...
  renderRail() {
    ...
    return `<aside class="control-rail ${compactRail ? "is-compact" : ""}" aria-label="${escapeHtml(this.t("a11y.controls"))}">
      ...
      <div class="control-stack ${this.state.mobileConfigOpen ? "is-mobile-open" : ""}">
        ...
        ${this.state.mode === "comparison" ? `<label class="field"><span>${escapeHtml(this.t("controls.runs"))}</span><input id="runs-input" type="number" min="2" max="64" step="1" value="${this.state.runs}" ${running ? "disabled" : ""}></label>
